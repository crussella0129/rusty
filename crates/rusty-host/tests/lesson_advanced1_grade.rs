//! End-to-end grading of advanced lesson 1 (Fearless Concurrency).
...
fn lesson_dir() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../content/lessons/advanced-01-concurrency")
}
...
fn sandbox_from(which: &str, tag: &str) -> PathBuf {
    ...
    let dst = std::env::temp_dir().join(format!("rusty_a1_{tag}_{nanos}_{n}"));
...
#[test]
fn test_advanced1_starter_fails() {
...
#[test]
fn test_advanced1_step2_faded_solution_passes() {
...
#[test]
fn test_advanced1_step3_faded_solution_passes() {
    ...
    let solved3 = solved2
        .replace("// tx.send(...).unwrap();", "tx.send(msg).unwrap();")
        .replace(
            "// let received = rx.recv().unwrap();",
            "let received = rx.recv().unwrap();",
        )
        .replace("String::new()", "received");
...
#[test]
fn test_advanced1_step4_open_solution_passes() {
