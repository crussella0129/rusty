...
## 2026-05-22 — Lesson body is an ordered `Step` list (supersedes the flat `body`+`exercises` shape) (sprint 5)
- **Context:** Progressive content disclosure (content materializes as the learner progresses) needs the lesson schema to express *gates* — but a flat `body: Vec<Block>` + separate `exercises: Vec<Exercise>` cannot say "this content is gated behind that exercise." This had to be settled before lessons 2–8 are authored or they'd need re-authoring.
- **Decision:** `Lesson.steps: Vec<Step>` replaces `body` + `exercises`. `Step { #[serde(default)] blocks: Vec<Block>, exercise: Option<Exercise>, hint: Option<String> }`. This **revises** the s2 curriculum-model ADR's *shape* but preserves its load-bearing properties: internally-tagged (`kind=`) enums (now nested in `[[steps.blocks]]` + an inline `success_criterion` — confirmed parseable with toml 1 / serde 1), the pure `parse_lesson(&str)` (FS stays in `rusty-host`), and lesson-project detach. Pure `Step::is_gating()` (Some Faded/Open) + `visible_prefix(steps, completed) -> usize` (prefix up to & incl. the first incomplete gating step) live in the portable crate.
- **Alternatives considered:** Keep `body`/`exercises` flat and bolt gating on by index convention — rejected: can't express which content belongs to which gate, fragile index math, still forces re-authoring later.
- **Consequences:** The gate is a first-class authoring unit. The migration was one atomic green commit (model + loader + both views + `main` + lesson 1 + host test). Non-rect future lessons author gates as data.

## 2026-05-22 — Exercise-gated progressive reveal: in-memory progress + fade-in; persistence deferred (sprint 5)
- **Context:** "The app looks for successful execution and moves on; gives a tip after the first incorrect try."
- **Decision:** `rusty-app` holds an in-memory `LessonProgress{completed, attempts}` (one slot per step). `lesson_view::render` shows only `steps[..visible_prefix(..)]`; a gating step's Check grades on the existing background thread, and on `Verdict::Pass` `apply` sets `completed[step]` (revealing the next step, which fades in via `ctx.animate_bool_with_time` + `ui.multiply_opacity`). A non-`Pass` bumps `attempts[step]`; after the first, the step's `hint` shows as a tip. Once `all_complete()`, a lesson-complete flourish + the (read-only) recall + further-reading reveal. **Progress persistence (`progress.json`) stays deferred to the old Phase 5** — progress resets each launch.
- **Alternatives considered:** Gating every prose step behind a click (too tedious — only Faded/Open gate); persisting progress now (separable; out of this sprint's scope).
- **Consequences:** The materialize-on-pass UX works in-session. The pure spine (`visible_prefix`/`apply`/`all_complete`/`tip_visible`/`pair_check`) is fully unit-tested; the fade + view-tree are heartbeat-verified (egui has no pixel/view assertion without a future `kittest` harness). **Phase 4 (rust-analyzer/LSP) was deferred** to run this ahead of it.

## 2026-05-23 — `LessonAction` over `Option<(usize, criterion)>` for lesson-pane → main signals (sprint 6)
- **Context:** Originally `lesson_view::render` returned `Option<(usize, SuccessCriterion)>` (a pressed Check's step+criterion). The ▶ run prompt needed to become interactive too, requiring a second signal type back to `main`. Stacking two separate `Option` returns was awkward.
- **Decision:** `pub struct LessonAction { pub check: Option<(usize, SuccessCriterion)>, pub run: Option<String> }` (derives `Default`, `Clone`, `Debug`, `PartialEq`, `Eq`). Both the per-step loop closure and the outer `render` return one; `main.rs::ui` consumes both fields independently. Adding future signals (e.g. "open lesson X" cross-lesson nav in Phase 5/6) is a struct-field addition with `Default` preserving back-compat.
- **Alternatives considered:** Multiple `&mut Option<T>` out-params (mutates aliasing, hard to test); a richer enum (locks the cardinality at one signal/frame — can't both Check and ▶ run in one frame). Rejected.
- **Consequences:** Sprint-6 byproduct: the refactor *appears* to have incidentally fixed the Reveal-Pass heisenbug (the static-analysis-only-impossible bug stopped reproducing after this change landed). Cause never identified; defended by the boundary panic-guard ADR below.

## 2026-05-23 — Boundary panic-guard: `enforce_gradeable_step` at the UI→grade chokepoint (sprint 6)
- **Context:** User reported a heisenbug where clicking PredictThenRun's "Reveal the answer" produced a Pass annotation, even though the static code path cannot route a Reveal click to `start_grade`. Sprint-6 instrumentation could not reproduce it after T-601's refactor.
- **Decision:** A pure free fn `fn enforce_gradeable_step(lesson: &Lesson, step: usize)` panics with `"start_grade boundary: step {step} is not gradeable (got {kind})"` if the step's exercise is not `Faded`/`Open`. Called at the single chokepoint in `main.rs::ui` — `if let Some((step, criterion)) = action.check { enforce_gradeable_step(lesson, step); self.start_grade(...); }` — turning a silent misroute into a loud, diagnostic-rich panic. 4 unit tests (`#[should_panic(expected = "step N is not gradeable (got KIND)")]` for Worked / PredictThenRun / no-exercise / out-of-range; ok for Faded / Open).
- **Alternatives considered:** Adding the guard inside `start_grade` (no `&Lesson` in scope; would require plumbing). Adding it inside the `lesson_view::render` pairing loop (`render_exercise` already type-narrows so the path is structurally unreachable there — assert would be theatre).
- **Consequences:** Recurrence of the *misroute* class of bug cannot be silent; the diagnostic payload (step index + variant name) tells future debuggers exactly which step and which variant slipped through. Recurrence of the *symptom* (Pass annotation without grade) outside the chokepoint is still possible in principle — addressed by the `LessonAction` ADR above and future `kittest`-based GUI tests.

## 2026-05-23 — Windows GUI-subsystem diagnostic pattern: trace-to-file mirror + chained panic hook (sprint 6, methodology ADR)
- **Context:** Sprint-6 mid-sprint discovery: `eprintln!` traces in a backgrounded `cargo run -p rusty-app` were swallowed by Windows GUI-subsystem stderr buffering when the app panic-exited (the assert fired but the lines never reached the capture). Heisenbug diagnosis stalled.
- **Decision (methodology, for future debugging sprints):** When instrumenting an egui-app for runtime tracing under headless capture:
  1. Define a `pub(crate) fn trace(msg)` that writes to BOTH stderr AND a stable file path (`std::env::temp_dir().join("<app>-trace.log")`), with `sync_all` after each write.
  2. Truncate the log at startup so each run reads cleanly.
  3. Chain `std::panic::take_hook` + `set_hook` so the panic info is `trace()`'d before the default hook unwinds.
  These three guarantee that diagnostic lines + panic context survive a Windows GUI-subsystem hard-exit. Add them when needed, remove with the instrumentation when done.
- **Consequences:** Recorded as durable methodology, not currently-in-code. Future Rusty diagnostic sprints (and any other Windows-egui project we touch) reach for this pattern without re-discovering the lesson.
